...
%   Update lists of input p-files and T1ws, and output Folder 
%   To run do: MRSCont = RunOspreyJob('PEACH_job_Thal.m')
%
...
% NOT using the BIDS loop because with multiple voxels and different
% sessions it's easier to define specific paths and use segmented anats for
% each voxel
...
%data_folder = '/Volumes/BIDS/CL_PEACH/';

% The following lines perform an automated set-up of the jobFile which
% takes advatage of the BIDS format. If you are not using BIDS (highly
...
% subs       = subs(contains({subs.name},sublist));
% counter    = 1;
% 
% 
...
%     sess        = sess(contains({sess.name},seslist));
%     for ll = 1:length(sess)
% 
%         % Specify metabolite data
%         % (MANDATORY)
%         dir_metabolite    = dir([sess(ll).folder filesep sess(ll).name filesep 'mrs' filesep subs(kk).name '_' sess(ll).name '*thal.7']);
...
% Specify metabolite data
files = {'/Volumes/BIDS/CL_PEACH/sub-31025/ses-03/mrs/sub-31025_ses-03_slaser_thal.7'};

% Specify T1-weighted structural imaging data
% Include both the full T1 and the segmented images when using the external
% segmentation results option!
% Link to single NIfTI (*.nii.gz or #.nii) files for GE, Siemens and Philips data
files_nii  = {'/Volumes/BIDS/CL_PEACH/sub-31025/ses-03/anat/sub-31025_ses-03_T1w.nii.gz'};

% External segmentation results
% run ACC script first to generate segmented T1s
% Link to NIfTI (*.nii or *.nii.gz) files with segmentation results
% Add supply gray matter, white matter, and CSF (c1, c2, c3 as 1 x 3 cell within a
% cell array  or a single 4D file in the same order supplied as 1 x 1 cell;
       files_seg = {{'/Volumes/BIDS/CL_PEACH/sub-31025/ses-03/anat/c1sub-31025_ses-03_T1w.nii'
                     '/Volumes/BIDS/CL_PEACH/sub-31025/ses-03/anat/c2sub-31025_ses-03_T1w.nii'
                     '/Volumes/BIDS/CL_PEACH/sub-31025/ses-03/anat/c3sub-31025_ses-03_T1w.nii'}};
...
% (MANDATORY)
outputFolder = '/Volumes/LaCie/Projects/mrs_development/PEACH/Thal/';

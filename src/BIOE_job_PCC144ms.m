...
data_folder = '/Volumes/LaCie/Projects/mrs_development/BIOE';
...
%         (MANDATORY)
       if exist([data_folder filesep subs(kk).name filesep 'mrs' filesep subs(kk).name '_slaser_pcc144ms.7']);
        dir_metabolite    = dir([data_folder filesep subs(kk).name filesep 'mrs' filesep subs(kk).name '*pcc144ms.7']); 
...
%         default.
%         dir_ref    = dir([sess(kk).folder filesep sess(kk).name filesep 'mrs' filesep subs(kk).name '_' sess(kk).name '_press-ref' filesep '*.SDAT']);
...
%         Specify metabolite-nukked data for quantification
%         (OPTIONAL)
%        files_mm     = {};
...
%         Link to DICOM (*.dcm) folders for GE data
       if exist([data_folder filesep subs(kk).name filesep 'mrs' filesep subs(kk).name '_slaser_pcc144ms.7']);
           files_nii(counter)  = {[data_folder filesep subs(kk).name filesep 'anat' filesep subs(kk).name '_rec-PURE_T1w.nii']};
       end
...
%         files_seg(counter)   = {{[sess(kk).folder filesep sess(kk).name filesep 'anat' filesep subs(kk).name filesep 'c1' sess(kk).name '_T1w.nii.gz'],...
%                                  [sess(kk).folder filesep sess(kk).name filesep 'anat' filesep subs(kk).name filesep 'c2' sess(kk).name '_T1w.nii.gz'],...
%                                  [sess(kk).folder filesep sess(kk).name filesep 'anat' filesep subs(kk).name filesep 'c3' sess(kk).name '_T1w.nii.gz']}};
% 
%         files_seg(counter)   = {{[sess(kk).folder filesep sess(kk).name filesep 'anat' filesep subs(kk).name filesep '4D' sess(kk).name '_T1w.nii.gz']}};
...
% Specify metabolite data
% (MANDATORY)
%files = {'/Volumes/LaCie/Projects/mrs_development/test_data/BIOE_DEV_EDITING/sub-BIOElac01/mrs/sub-BIOElac01_sl_te288_P18432.7'
 %        };
...
%files_nii  = {'/Volumes/LaCie/Projects/mrs_development/test_data/BIOE_DEV_EDITING/sub-BIOElac01/anat/sub-BIOElac01_T1w.nii'
%              };
...
% Add supply gray matter, white matter, and CSF as 1 x 3 cell within a
% cell array  or a single 4D file in the same order supplied as 1 x 1 cell;
%         files_seg = {{'/Volumes/LaCie/Projects/mrs_development/HERMES/BIO_TEST_0503/anat/c1BIO_TEST_0503_T1',
%                                  '/Volumes/LaCie/Projects/mrs_development/HERMES/BIO_TEST_0503/anat/c2BIO_TEST_0503_T1',
%                                  '/Volumes/LaCie/Projects/mrs_development/HERMES/BIO_TEST_0503/anat/c3BIO_TEST_0503_T1'}};
        
%         files_seg = {'/Volumes/LaCie/Projects/mrs_development/HERMES/BIO_TEST_0503/anat/BIO_TEST_0503_T1.nii'};
...
% Specify output folder (you can always use the direct path, must include final /)
% (MANDATORY) 
outputFolder = '/Volumes/LaCie/Projects/mrs_development/BIOE_test_jan2025/pcc144ms/';

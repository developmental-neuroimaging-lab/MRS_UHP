...
%   To run do: MRSCont = RunOspreyJob('PEACH_job_ACC.m')
...
% NOT using BIDS looping because it is difficult to specify all the
% specific sessions, voxels, etc.
...
%         % (MANDATORY)
%         dir_metabolite    = dir([sess(ll).folder filesep sess(ll).name filesep 'mrs' filesep subs(kk).name '_' sess(ll).name '*acc.7']);
...
% Specify metabolite data
files = {'/Volumes/BIDS/CL_PEACH/sub-31025/ses-03/mrs/sub-31025_ses-03_slaser_acc.7'};
...
% run ACC script first to generate segmented T1s, exclude this option
...
% (MANDATORY)
outputFolder = '/Volumes/LaCie/Projects/mrs_development/PEACH/ACC/';
